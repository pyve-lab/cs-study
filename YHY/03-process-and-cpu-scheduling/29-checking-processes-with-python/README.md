...
- [🧠 29강 파이썬 코드로 프로세스 확인하기](#-29강-파이썬-코드로-프로세스-확인하기)
  - [📚 목차](#-목차)
  - [1) 준비 지식: PID / PPID란?](#1-준비-지식-pid--ppid란)
  - [2) os.getpid()로 내 프로세스 PID 확인](#2-osgetpid로-내-프로세스-pid-확인)
    - [✅ 기대되는 흐름](#-기대되는-흐름)
  - [3) Process 1개 생성: 부모/자식 PID 관계 확인](#3-process-1개-생성-부모자식-pid-관계-확인)
    - [✅ 코드 설명](#-코드-설명)
    - [✅ 출력에서 확인할 것](#-출력에서-확인할-것)
  - [4) Process 3개 생성: 여러 자식 프로세스 동시에 실행](#4-process-3개-생성-여러-자식-프로세스-동시에-실행)
    - [✅ 핵심 포인트](#-핵심-포인트)
    - [✅ 관찰 포인트](#-관찰-포인트)
  - [5) 같은 함수(foo)를 3번 실행](#5-같은-함수foo를-3번-실행)
    - [✅ 무엇을 확인하는 코드인가?](#-무엇을-확인하는-코드인가)
  - [6) 서로 다른 함수(foo/bar/baz)를 각각 실행](#6-서로-다른-함수foobarbaz를-각각-실행)
    - [✅ 쉽게 이해하기](#-쉽게-이해하기)
    - [✅ 실무적으로 연결하면](#-실무적으로-연결하면)
  - [7) 자주 하는 실수 / 체크포인트](#7-자주-하는-실수--체크포인트)
    - [1) `if __name__ == '__main__':` 생략](#1-if-__name__--__main__-생략)
    - [2) `.start()`의 반환값 오해](#2-start의-반환값-오해)
    - [3) 출력 순서가 매번 달라짐](#3-출력-순서가-매번-달라짐)
...
## 3) Process 1개 생성: 부모/자식 PID 관계 확인
...
## 4) Process 3개 생성: 여러 자식 프로세스 동시에 실행
...
## 6) 서로 다른 함수(foo/bar/baz)를 각각 실행
